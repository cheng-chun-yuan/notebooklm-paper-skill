...
$PAPER_SKILL/bin/paper-update-check

# Apply update
cd $PAPER_SKILL && git pull origin main

# If requirements.txt changed, reinstall dependencies
$PY -m pip install -r $PAPER_SKILL/requirements.txt
```

## Workflow

1. Run `$PAPER_SKILL/bin/paper-update-check` to see if an update is available
2. If output contains `UPGRADE_AVAILABLE`, inform the user of the current and available versions
3. If the user confirms, run `cd $PAPER_SKILL && git pull origin main`
4. After pulling, check if `requirements.txt` was modified in the update:
   ```bash
   cd $PAPER_SKILL && git diff HEAD~1 --name-only | grep requirements.txt
   ```
5. If `requirements.txt` changed, run `$PY -m pip install -r $PAPER_SKILL/requirements.txt`
6. Report the new version from `$PAPER_SKILL/VERSION`
