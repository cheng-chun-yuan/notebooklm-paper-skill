...
Papers are reusable across projects. A paper analyzed for project A might be relevant to project B. `~/.paper/` is the research database. Each project gets its own directory under `~/.paper/projects/{name}/` with phase artifacts, cross-cutting JSON files, and drafts.

Project-local linking: when working in a project directory, `RESEARCH_SYNTHESIS.md` is generated there (committed to git), and `.paper/project.json` links back to the global project (gitignored).
