...
All artifacts from `~/.notebooklm-paper-skill/projects/{name}/`:
...
Read all artifacts from `~/.notebooklm-paper-skill/projects/{name}/`. This phase has two stages — run them in order.
...
```
~/.notebooklm-paper-skill/projects/{name}/critique-report.md
```
